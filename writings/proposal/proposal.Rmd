...
The data set used in this project contains experimental test data from studies about human learning and explorative behavior of humans.
...
The overall recorded data set consists of the trajectory data of 66 test persons in the virtual world as well as several other variables such as sex, age and others. The data set is divided into trajectory data and informations of the test persons in separate csv files.
...
* What is the impact of the novelty of the virtual world on the learning success on day two (How did the number of remembered words change depending wether the test persons have seen an old or a new world before memorizing the vocabularies? Is there a difference for ADHD and non-ADHD persons?)
...
For the feature description using the architecture of the virtual world (rooms etc.) a conversion from trajectory data given in euclidean coordinates must be converted to a graph like representation listing the order and time spent in rooms.
After the derivation of trajectorie features those will be used to predict or correlate to the test persons attributes retrieved by questionnaires. This tasks makes particular use of clustering algorithms.
...
The trajectory date will be visualize using 3D scatter plots. We will provide a visual comparison method of several trajectories from different test persons. We seek an embedding of the architecture of the virtual world into the scatterplot visualization.
...
The table data will be displayed separately using a scatter plot matrix to allow analysis of correspondence and variance of the data. For the scatter plot matrix a way to filter into groups (e.g. only test persons with ADHD) will be provided.
...
|                 |                   |exclude broken trajectory data entries           |             |
| 26.11. - 02.12. |feature extraction  |identify possible trajectory features |             |
|                 |                     |extract features from trajectory                             |             |
|                 |                     |scale and process features to make them suitable for algorithms|             |
| 03.12. - 09.12. |alogrithm                    |           |             |
|                 |                    |           |             |
|                 |                    |           |             |
|                 |                    |           |             |
|                 |                    |           |             |
