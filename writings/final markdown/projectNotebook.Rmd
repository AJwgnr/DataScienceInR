...
The packages are listet in the order they are loaded.
We wont go into detail about every package since there is a lot of information available on all of them online.
To mention are shiny and the various extensions used to create the interactive environment. The data.table package is an extension to the inbuild data.frame but can handel large data sets faster, reads data faster and provides an easier way of indexing and subsetting data.tabels.
We highlighted the plotly package not only because it provides an easy way to create interactive plots but also we prefered it over ggplot2 which we used only for special cases.
Probably the most unknown important package in the list is trajr. It is originally designd to analyse 2d animal trajectory. Althou our data contains 3d trajectory data the trajr package has provided usefull in feature computation.
The packages not mentioned here are used for minor computations throuout the program and could be replaced with similar in most cases.
...
The test persons data table described in 1.4.1 is the first data loaded into a data.table object using the fread function of the beloning package. The data table contains all the attributes listet in 1.4.1. Those attributes and column names remain unchanged throughout the program. The persons data table is the most crucial part of data and used for various plots and analysis tasks. 

The second data loaded into are the trajectory files stored in csv format. Since they the trajectory files contain a different number of rows each and are based on several virtual minecraft worlds they are loaded and stored into two lists. One for each day indexed by the persons VP number. There is no seperation made between the different virtual worlds at this step. The trajectories them selfes are stored as data.table using fread to gain a significant speed up compared to the inbuild loading function and storage into data.frame.

The last data loaded from file by default are two files containing the geometry of the virtual worlds. There are three worlds but only differ geometrically where as world one and three only differ in color and apperance.
...
Since the geometrie of world one and three is equal the number of rooms is equal as well. For the worlds one and two this does not hold. 
The geometrie only allows for rectangular rooms consiting of the lower vertex (x1,y1) and the upper vertex (x2,y2) spanning the rectangle and holding x1<x2 and y1<y2. This is checked and corrected within the loading function. 
...
The following charts are dealing with the analyzation of the results of the memorizing experiment. On top of this website tab the most important figures of the experiment are shown:

* Patient had to recall 20 vocabularies
...
Furthermore the bar charts below those figures show the distribution of the ADHD type of the patient with respect to the three different groups.
...
The following four boxplots show the comparison of the average TP_Direct_Recall and the TP_Delayed_Recall with respect to the ADHD and control group patient. 

* Group 1: Children having only seen the same world twice
* Group 2: Children having seen a new world on the second day
* Group 3: Children having seen a partial new world on the second day 
...
* We created a shiny web app
* we abstracted basic functionalities for trajectorie processing and feature calculation into several .R files
* we encapsulated the visualization from the underlying data (data is accessed dynamically within an .R file und just needs to be changed/added)
