...
The data set of the test persons contains variables such as sex, age, and others as well as the number of words remembered correctly/wrongly on the first/second day, the virtual world id for day one/two and several scores obtained using a questionnaire related to immersion, exploration behavior and similar. The full list of attributes of the used data sets is listed below.
...
To mention are shiny and the various extensions used to create the interactive environment. The data.table package is an extension to the inbuilt data.frame but can handle large data sets faster, reads data faster and provides an easier way of indexing and sub setting data.tabels.
...
Since there were only few test persons from begin on we decided not to remove those prior to the analysis. We suggest implementing a global filter option to exclude this test persons from analysis but requires several changes to the code foundation and is thus postponed.
...
The current version of the worlds geometry contains minor errors in the room coordinates. Especially the second world (PirateShip) is not working properly throughout all z levels due to the complex geometry containing lots of round features and slopes.
...
The id and name of the room both identify the room but can occur multiple time in the geometry file to form non rectangular rooms. The id counts all the rooms from zero to the total number of rooms and contains minus one as a special id reserved as an entry for trajectory coordinates outside all room coordinates.
...
However we take no responsibility or give warranty of any kind.
...
Besides the option of plotting only two dependent variables, the webpage also offers the option to create a scatterplotmatrix of as many attributes as desired (See the next image). A short drawback is the problem that Shiny is not capable of rendering huge axis labels correctly which can disturb the analysis when many features are used.
...
* Screen cast of the application doesn´t really make sense (for an interactive web app at least)
