...
* Fewer people in the partial new group -> also only control group there
* less ADHD test persons than persons in the control group
...
On the very top the tab allows for the selection of up to three filters applyable to the data set piror to plotting.
...
The two version of the Mansion are treated as the same since the geometry is identicall.
The group has been collapsed into Control and ADHD since the ADHD suptypes cause a splitting into unresonable small bins.
Due to the interactiveness further filters or the selection of an attribute from persons data table for filtering can ben implemented with relative ease.
...
It is questionable how much information can be gaind by analyzing the time spent per room.

![Percentage of rooms the test person have explored](../images/percentOfRoomsVisited.png)

**Important Insights:**

* With over 50% minimum room coverage on day two this is worth mentioning taking the complex geometry and short amout of exploration time into account
* On both worlds the median coverage of rooms is higher for the control group (79.6% vs 84.6%) regardless of days

![Rooms visited multiple times](../images/roomsMultipleTimes.png)
...
* Wuuhuuuu
* Clustering without resonable features and few data points is crappy
* percentage
