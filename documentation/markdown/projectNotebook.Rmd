...
The data preprocessing could be done manually for the persons data table introduced in 1.4.1 and consisted mainly of the removal of ill suited entries and filling in of empty data where possible.
The reason rows or test persons had to be excluded was the missing of trajectory data for those and impacted two entries. The filling of missing entries could be done for columns like VE_Day2 in cases where the missing was redundant and computable by looking at firstVR and Novelty columns.
Another preprocessing step was the renameing and unifing of the trajectory fileanmes and the removal of trajectory files of test persons not listet in the persons data table.

The most crucial and lager preprocessing step was to manually retrieve the room coordinates of the two world geometries. This includes the exploration of the minecraft world, cutting the world geometry into rectangular patches and noting the coordinates and id of the patches.
...
Because the computation, especially the conversion from trajectory to room graph description takes a lot of time. Therefore the files are both stored in /precomputed and only computed if not accessible for loading. Currently it does not contain any functionality to detect changes to the data loading or processing and thus the precomputed data must be removed by hand to force the recomputation. This should be fixed to avoid possible future errors.

One more conversion of the trajectory data was introduced in the data processing step to allow the computation of trajectory features using the trajr package.
It is important to note the omitting of the z coordinate. This is forced by trajr which only allows for 2d trajectories. The conversion is debatable and may lead to wrong conclusions but for as a prove of concept shows that it is in general possible to use trajr.
There is no precomputation done prior to conversion to tarjr expect the omitting of z. We are aware of the error this creates especially around stairways and due to false intersections introduced by collapsing the trajectory along z. However be thing this is acceptable since the movement is mostly along the x-y plane and players are bound to walk and have no means of flying or similar.
...
Since the trajr object contains similar data compared to the trajectory data described previous and is well documented we will not further discuss it here.

The last step worth mentioning in the computation of data before analysis and visualization tasks is the computation of trajectory features to allow a comparison of trajectories and as such clustering and others.

Therefore we already provided the room graph an room hist data.table as well as the conversion to trajr trajectories. From these data sets we computed the following features:
...
The features are computed using the above stated data.tables for each day and person separately. Instead of storing the features in a list they are simply added as columns to the test persons data.table and read from there. The corrected sinuosity and straightness features are computed using trajr functions.

## 3.3 Algorithms and Methods

We are applying different machine learning and visualization techniqeus to gain new insights from the data. Table views are used to allow an interactive exploration of the raw dataset. By using scatterplot matrixes and linear regression, linear relationship between attributes can be identified. 
Furthermore 3D-plots of the trajectories are used to not only get an impression on how the person moved in the world but also to compare the different movements on either different days or different persons. Several bar charts are used to plot more detailed information about the trajectory files like the average time a person has spent in each room of the world or how often a room was entered. The comparison of features of different group of test persons is done usin boxplots. The results of all those algorithms and visualizations can be viewed interactively on our webpage but also in section 4 Results in this markdown document.
...
Since the analysis and computation of the trajectury features as well as the building of the shiny webapp was quite complicated our project lacks further machine learning methods. We only applied XXXXXX.
