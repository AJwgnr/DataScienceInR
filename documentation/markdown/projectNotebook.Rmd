...
It is the task of the user to choose meaningful attributes.
...
One of the goals was to cluster the person data attributes such as the TP_recall, VLMT and NV measures on one hand and the features retrieved from the trajectory data set such straightness, roughness. The resulting clusters should be compared to one another with respect of distance between the test persons in each room and cluster membership. The clustering results would also have been compared to attributes like ADHD_Subtype.
...
Several problems arose during the analysis forcing us to drop the idea of clustering completely.
First of all the data set contained considerable few test persons to cluster on.
The Exclusion_Position_Data column showed problems in the logging of several trajectory files further reducing the number of data points available for clustering.
...
During the analysis of the data attributes like age and sex were dropped from clustering since the clustering results were expected to be not useful. Other attributes like Gaming_Experience or Novelty had to be dropped out because of a lack of variance or consisting of True/False entries only.
With the few remaining attributes clustering was error prone and the clustering results were unstable and showed no meaningful insight for further investigation.
For the trajectory based features the result was similar in the end. The trjr package seemed promising first but was only able to work on 2d data reducing the reliability of the computed features.
...
The clustering result on those was as useless as the prior one.
...
* Construct meaningful features like roughness, steadiness, periodicity, size
