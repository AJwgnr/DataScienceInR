...
shinyServer(function(input, output, session) {
  ...
  output$gx_3d_trajectoryDayOne <- renderPlotly({
    ...
    if (length(selectedPersons)) {
      ...
      # TODO: highligth room geometry, fix aspec ratio
      # VERY IMPORTANT: Override trjPlotDayOne here, otherwise it only works if no single line is added after all the piping... ez gg
      ...
      if (vr == 1 || vr == 3)
      {
        trjPlotDayOne <- trjPlotDayOne %>%
          add_trace(
            ...
            j = c(
              ...
              (2 *
                 nrow(roomCoordinatesVR1.0)):(3*nrow(roomCoordinatesVR1.0)-1)
            ),
            k = c((3 * nrow(
              roomCoordinatesVR1.0
            )):(4 * nrow(
        ...
        trjPlotDayOne <- trjPlotDayOne %>%
          add_trace(
            ...
            j = c(
              ...
              (2 *
                 nrow(roomCoordinatesVR2.0)):(3*nrow(roomCoordinatesVR1.0)-1)
            ),
            k = c((3 * nrow(
              roomCoordinatesVR2.0
            )):(4 * nrow(
        ...
        print("Wrong VR ID provided in trajectory plotting. Was none of  1,2,3")
        print(toString(vr))
      }
      
      trjPlotDayOne %>% layout(title = "Propands path thru the VR")
    }
  })
